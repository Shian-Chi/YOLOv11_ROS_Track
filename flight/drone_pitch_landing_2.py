...
def fly_to_global(pub : DronePublishNode, sub : DroneSubscribeNode, cli : DroneClientNode,latitude, longitude, altitude, delta_yaw, origin_lat, origin_lon):
    ...
    #temp_target_yaw = pub.alwaysSendPosGlobal.yaw
    ...
    if sub.detect == True: #辨識到目標
        ...
        # else :
        #     print("-------------------target center failed!------------------------")
        #     pub.alwaysSendPosGlobal.yaw = temp_target_yaw
        #     time.sleep(5)
        return True
...
def landing_tracking(pub : DronePublishNode, sub : DroneSubscribeNode):
    ...
    while(True):
        ...
        if(sub.motor_pitch > 90):
            #退後
            delta_y = 0.5 * math.sin(math.radians(sub.yaw))
            delta_x = 0.5 * math.cos(math.radians(sub.yaw))
        ...
        if(abs(sub.motor_yaw) > 5):
            #先判斷雲台yaw角度是正(向右飛)還是負(向左飛)
            if(sub.motor_yaw) > 0:
                #向右飛
                temp_yaw = sub.yaw - 90.0
                if temp_yaw < 0:
        ...
        pub.alwaysSendPosGlobal.latitude = target_lat
        pub.alwaysSendPosGlobal.longitude = target_lon
        pub.alwaysSendPosGlobal.altitude =  sub.get_altitude() - 5.0
                    ...
                    for row in range(len(drone_point)):
                        if row == 0:
                            ...
                            if droneSub.detect == True:
                                ...
                                #向前飛，使得雲台的Pitch垂直於目標
                                drone_moving_along_the_x(dronePub, droneSub, droneSub.heading)
                                landing_tracking(dronePub, droneSub)
                                temp_status = True
                                droneCli.requestLand()
                                break
