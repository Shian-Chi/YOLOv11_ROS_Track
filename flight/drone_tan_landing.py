...
class DronePublishNode(Node):
    def __init__(self, freqPositionLocal):
        ...
        self.hold_status = False
        self.detection_forward_yaw_correction_status = False
...
def fly_to_global(pub : DronePublishNode, sub : DroneSubscribeNode, cli : DroneClientNode,latitude, longitude, altitude, delta_yaw, origin_lat, origin_lon):
    ...
    if (latitude != 0.0) and (longitude != 0.0):
        while (((abs(sub.latitude - latitude)*110936.2 > 3) or (abs(sub.longitude - longitude)*101775.45 > 3)) and (sub.detect == False)):  #or (abs(sub.altitude - altitude) > 1.5)
...
def fly_to_global_without_detect(pub : DronePublishNode, sub : DroneSubscribeNode, latitude, longitude, altitude, delta_yaw):
    ...
    if (latitude != 0.0) and (longitude != 0.0):
        while (((abs(sub.latitude - latitude)*110936.2 > 3) or (abs(sub.longitude - longitude)*101775.45 > 3))):  #or (abs(sub.altitude - altitude) > 1.5)
            print("+++++++lat distance:", abs(sub.latitude - latitude)*110936.2, "lng distance:", abs(sub.longitude - longitude)*101775.45, "alt distance:", abs(sub.altitude - altitude))
            time.sleep(0.1)
...
def takeoff_global(pub : DronePublishNode, sub : DroneSubscribeNode, srv : DroneClientNode, rel_alt : float):
    ...
    data.yaw = degree_conv_radian(0.0) #degree_conv_radian(sub.yaw)
...
def drone_moving_along_the_x(pub : DronePublishNode, sub : DroneSubscribeNode, origin_heading, altitude):
    ...
    if origin_heading < 0:
        origin_heading = 360 - origin_heading

    theta_radians = math.radians(origin_heading)
    ...
    delta_lat = forward_distance*delta_x*(1/110936.32)
        ...
        if droneState.droneState == 1: #groundControlCommand.DRONE_TAKEOFF.value:
            ...
            while True:
                if droneState.droneState == 2:
                    ...
                    print(bearing)
                    fly_to_global_without_detect(dronePub, droneSub, origin_latitude, origin_longitude, drone_point[0][1], bearing)
                    print("轉向前進方位角完成")
                    ...
                    for row in range(len(drone_point)):
                        if row == 0:
                            print("first point")
                            a = fly_to_global(dronePub, droneSub, droneCli, drone_point[row][2], drone_point[row][3], drone_point[row][1], 0.0, origin_latitude, origin_longitude)
                            if a == True:
                                print("land test1")
                                ...
                                landing_process(dronePub, droneSub, takeoffAltitude)	
                            ...
                            print("new point")
                            print(row)
                            a = fly_to_global(dronePub, droneSub, droneCli, drone_point[row][2], drone_point[row][3], drone_point[row][1], drone_point[row][4], origin_latitude, origin_longitude)
                            if a == True:
                                print("land test111")
                                #fly_to_global(dronePub, droneSub, droneCli, origin_latitude, origin_longitude, takeoffAltitude, bearing, origin_latitude, origin_longitude) #懸停
                                print('33333333')
